---
name: agent-skill-testing
description: "エージェントが生成・実行するスクリプト（スキル）のテスト設計指針。シェルスクリプト・ファイルシステム操作・AI生成コードのテストに関する知見をまとめる。キーワード: スキルテスト, シェルスクリプトテスト, ファジング, テスト設計, エージェントテスト"
---

# agent-skill-testing - エージェントスキルのテスト設計指針

Claude Code のスキル（主にシェルスクリプト）を開発・テストする際の設計指針。
実際のスキル開発（move-project）と Codex (GPT-5.3) のレビューから蒸留した知見。

## When to Use

- 新しいスキル（.sh / SKILL.md）を作るとき
- スクリプトのテストケースを設計するとき
- 「テストしたのに実環境で壊れた」を防ぎたいとき
