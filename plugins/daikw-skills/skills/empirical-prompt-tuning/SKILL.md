...
**usage メタが取得できない場合のフォールバック**: バックグラウンド起動や一部環境では Task tool の usage メタ（`tool_uses` / `duration_ms`）が取得できず定量軸が全滅する。この場合、実行者への依頼プロンプトに「使用したツール呼び出しの概数」を自己申告レポートへ含めさせ、ステップ数の代替とする。自己申告なので精度は落ちる（過小・過大申告のバイアスがあり得る）ことを評価時に注記する。
...
実行者は原則 **同期起動**（Task tool をバックグラウンドにしない）で dispatch する。バックグラウンド起動すると、最終レポート（自己申告の不明瞭点・裁量補完）が呼び出し側に自動で返らず、催促してもタスク名からの推定に頼ることになる（EasyEDA skill 改善時の実例）。質的評価が主軸のため、回収不安定は致命的。

やむを得ずバックグラウンド起動する場合は、後述レポート構造の末尾に「レポート全文を <呼び出し側が指定する scratchpad パス> に Write せよ」という一文を実行者へのプロンプトに含める。呼び出し側は完了通知後、そのファイルを Read で回収する（通知本文からの推定に頼らない）。
...
- **ステートフル共有環境の直列化を怠る**: 対象プロンプトが単一のライブ環境（GUI アプリ・実機・共有 DB 等）を操作する場合、シナリオの並列実行は禁止し直列で回す。並列化して良いのは環境インスタンスを分離できる場合（別ウィンドウ・別プロジェクト・別 DB）のみ。実例: EasyEDA 単一ウィンドウに 2 エージェントが同時書き込みし成果物が相互汚染、DRC 結果が信頼不能になった。
